...
# Load Training Data
xtrain, ipVec_dim, nChannels = get_mnist_dataloader(args=opt) #! Load MNIST data using the parsed data above
...
while cost > 0.2 and t <= opt.max_epochs:  # run epochs until convergence
    ...
    for i, (datax, datay) in enumerate(xtrain):
        if i < math.ceil(opt.N / opt.mb_size):
            ...
            output1 = net1(datax) #! Project data X on new space using kernel
            output2 = net2(datay) #! Project data y (labels) on new space using kernel
